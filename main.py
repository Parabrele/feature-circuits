if __name__ == "__main__":
    ...
    parser.add_argument("--eval_method", "-em", type=str, default="edge_patching")
    parser.add_argument("--eval_metric", "-emt", type=str, nargs='+', default=["logit", "KL"])
    parser.add_argument("--eval_batch_size", "-ebs", type=int, default=1)
    parser.add_argument("--eval_start_at_layer", "-esal", type=int, default=2)
    ...
    parser.add_argument("--get_circuit", "-gc", type=bool, default=True)
    parser.add_argument("--eval_circuit", "-ec", type=bool, default=True)
    parser.add_argument("--marks_version", "-mv", type=bool, default=False)

    args = parser.parse_args()

    available_cm = ["resid", "marks", "resid_topk"]
    available_em = ["node_patching", "edge_patching"]
    available_emt = ["logit", "KL"] # TODO : add more (acc, 1/rank, prob, logprob, ...)
    ...
    def plot_faithfulness(outs):
        ...
        fig.save(save_path + "faithfulness.html")
    ...
    def run_second_step_marks_version():
        ...
        circuit = load_latest(circuit_path)
    ...
    # TODO : load circuit from circuit_path, if several devices available, merge the latest of each.
    if run_step_2:
        if marks_version:
